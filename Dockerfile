...
RUN FFMPEG_DIR=$(tar -ztfJ ffmpeg-git-amd64-static.tar.xz | egrep '^[^/]+/?$') && echo $FFMPEG_DIR
USER root
RUN FFMPEG_DIR=$(tar -ztfJ ffmpeg-git-amd64-static.tar.xz | egrep '^[^/]+/?$') && tar xvfJ ffmpeg-git-amd64-static.tar.xz && ln $FFMPEG_DIR/ffmpeg /usr/local/bin/ffmpeg
