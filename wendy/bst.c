...
struct node * bst_insert(struct node * node, int idx, double val){
  // Empty tree: return single new node
  if (node == NULL) return bst_newNode(idx,val);
  // Build tree using recurrence
  if (val < node->val)
    node->left= bst_insert(node->left,idx,val);
  else if (val > node->val)
    node->right= bst_insert(node->right,idx,val);
  return node;
}
...
struct node * bst_build(int N,int * idxs, double * vals){
  ...
  int ii;
  for (ii=0; ii < N; ii++)
    root= bst_insert(root,*(idxs+ii),*(vals+ii));
  return root;
}
